...
def lcs_backtrack_3d(
    ...
    backtrack = np.zeros(
        (len(str_1) + 1, len(str_2) + 1, len(str_3) + 1), dtype=np.int64
    )
    for i in range(1, len(str_1) + 1):
    ...
    for i in range(1, len(str_1) + 1):
        for j in range(1, len(str_2) + 1):
            s[i, j, 0] = s[i - 1, j - 1, 0] - indel_penalty
            backtrack[i, j, 0] = 4
    for i in range(1, len(str_1) + 1):
        for k in range(1, len(str_3) + 1):
            s[i, 0, k] = s[i - 1, 0, k - 1] - indel_penalty
            backtrack[i, 0, k] = 5
    for j in range(1, len(str_2) + 1):
        for k in range(1, len(str_3) + 1):
            s[0, j, k] = s[0, j - 1, k - 1] - indel_penalty
            backtrack[0, j, k] = 6
    ...
    for i in range(1, len(str_1) + 1):
        for j in range(1, len(str_2) + 1):
            for k in range(1, len(str_3) + 1):
                match = (
                    match_reward
                    if str_1[i - 1] == str_2[j - 1] and str_2[j - 1] == str_3[k - 1]
                ...
                if s[i, j, k] == s[i - 1, j, k] - indel_penalty:
                    backtrack[i, j, k] = 1
                elif s[i, j, k] == s[i, j - 1, k] - indel_penalty:
                    backtrack[i, j, k] = 2
                elif s[i, j, k] == s[i, j, k - 1] - indel_penalty:
                    backtrack[i, j, k] = 3
                elif s[i, j, k] == s[i - 1, j - 1, k] - indel_penalty:
                    backtrack[i, j, k] = 4
                elif s[i, j, k] == s[i - 1, j, k - 1] - indel_penalty:
                    backtrack[i, j, k] = 5
                elif s[i, j, k] == s[i, j - 1, k - 1] - indel_penalty:
                    backtrack[i, j, k] = 6
                else:
                    backtrack[i, j, k] = 7
    # print(backtrack)
...
def output_lcs_3d(
    ...
    score, backtrack = lcs_backtrack_3d(
        str_1, str_2, str_3, match_reward, mismatch_penalty, indel_penalty
    )

    def __output_lcs1(backtrack, str_1, i, j, k):
        if i == 0 and j == 0 and k == 0:
            return ""
        if backtrack[i, j, k] == 0:
            return ""
        elif backtrack[i, j, k] == 1:
            return __output_lcs1(backtrack, str_1, i - 1, j, k) + str_1[i - 1]
        elif backtrack[i, j, k] == 2:
            return __output_lcs1(backtrack, str_1, i, j - 1, k) + "-"
        elif backtrack[i, j, k] == 3:
            return __output_lcs1(backtrack, str_1, i, j, k - 1) + "-"
        elif backtrack[i, j, k] == 4:
            return __output_lcs1(backtrack, str_1, i - 1, j - 1, k) + str_1[i - 1]
        elif backtrack[i, j, k] == 5:
            return __output_lcs1(backtrack, str_1, i - 1, j, k - 1) + str_1[i - 1]
        elif backtrack[i, j, k] == 6:
            return __output_lcs1(backtrack, str_1, i, j - 1, k - 1) + "-"
        else:
            return __output_lcs1(backtrack, str_1, i - 1, j - 1, k - 1) + str_1[i - 1]

    def __output_lcs2(backtrack, str_2, i, j, k):
        if i == 0 and j == 0 and k == 0:
            return ""
        if backtrack[i, j, k] == 0:
            return ""
        elif backtrack[i, j, k] == 1:
            return __output_lcs2(backtrack, str_2, i - 1, j, k) + "-"
        elif backtrack[i, j, k] == 2:
            return __output_lcs2(backtrack, str_2, i, j - 1, k) + str_2[j - 1]
        elif backtrack[i, j, k] == 3:
            return __output_lcs2(backtrack, str_2, i, j, k - 1) + "-"
        elif backtrack[i, j, k] == 4:
            return __output_lcs2(backtrack, str_2, i - 1, j - 1, k) + str_2[j - 1]
        elif backtrack[i, j, k] == 5:
            return __output_lcs2(backtrack, str_2, i - 1, j, k - 1) + "-"
        elif backtrack[i, j, k] == 6:
            return __output_lcs2(backtrack, str_2, i, j - 1, k - 1) + str_2[j - 1]
        else:
            return __output_lcs2(backtrack, str_2, i - 1, j - 1, k - 1) + str_2[j - 1]

    def __output_lcs3(backtrack, str_3, i, j, k):
        if i == 0 and j == 0 and k == 0:
            return ""
        if backtrack[i, j, k] == 0:
            return ""
        elif backtrack[i, j, k] == 1:
            return __output_lcs3(backtrack, str_3, i - 1, j, k) + "-"
        elif backtrack[i, j, k] == 2:
            return __output_lcs3(backtrack, str_3, i, j - 1, k) + "-"
        elif backtrack[i, j, k] == 3:
            return __output_lcs3(backtrack, str_3, i, j, k - 1) + str_3[k - 1]
        elif backtrack[i, j, k] == 4:
            return __output_lcs3(backtrack, str_3, i - 1, j - 1, k) + "-"
        elif backtrack[i, j, k] == 5:
            return __output_lcs3(backtrack, str_3, i - 1, j, k - 1) + str_3[k - 1]
        elif backtrack[i, j, k] == 6:
            return __output_lcs3(backtrack, str_3, i, j - 1, k - 1) + str_3[k - 1]
        else:
            return __output_lcs3(backtrack, str_3, i - 1, j - 1, k - 1) + str_3[k - 1]

    return (
        score,
        __output_lcs1(backtrack, str_1, len(str_1), len(str_2), len(str_3)),
        __output_lcs2(backtrack, str_2, len(str_1), len(str_2), len(str_3)),
        __output_lcs3(backtrack, str_3, len(str_1), len(str_2), len(str_3)),
    )
